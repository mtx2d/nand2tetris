...
def get_files(input_path):
    def is_vm_source_file(file_path):
        return file_path.endswith(".vm") and os.path.isfile(os.path.join(input_path, file_path))

    ans = [os.path.join(input_path, p) for p in os.listdir(input_path) if is_vm_source_file(p)]
    return ans
